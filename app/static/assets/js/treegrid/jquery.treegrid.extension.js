            ...
            $.each(data, function (i, item) {
                if (item[options.parentColumn] == parentNode[options.id]) {
                    ...
                    $.each(options.columns, function (index, column) {
                        var td = $('<td></td>');
                        td.html("<div> <button type='button' class = 'btn btn-info btn-sm edit-btn' data-toggle='modal' data-target='#updateModal'" +
                        "onclick='setId("+item["id"]+")' id='update"+item["id"]+"' style='font-size: 12px; padding: 2px 10px;' >修改</button></div>");
